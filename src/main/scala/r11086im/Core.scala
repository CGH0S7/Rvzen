...
class Core extends Module {
  ...
  val imm_s_sext = Cat(Fill(12, imm_s(11)), imm_s)
  val imm_b = Cat(inst(31), inst(7), inst(30, 25), inst(11, 8))
  val imm_b_sext =
    Cat(Fill(19, imm_b(11)), imm_b, 0.U(1.W)) // imm_b本身只有12位因此扩展的是imm_b(11)
  val imm_j = Cat(inst(31), inst(19, 12), inst(20), inst(30, 21))
  val imm_j_sext = Cat(Fill(11, imm_j(19)), imm_j, 0.U(1.W)) // 最低位归零
  ...
  val op1_data =
    MuxCase(
      ...
      Seq(
        ...
        (op1_sel === OP1_PC) -> pc_reg // 添加OP1_PC的情况
      )
    )
  ...
  alu_out := MuxCase(
    ...
    Seq(
      ...
      (exe_fun === ALU_MUL) -> (op1_data * op2_data),
      (exe_fun === ALU_DIV) -> (op1_data / op2_data),
      (exe_fun === ALU_MULH) -> (op1_data.asSInt * op2_data.asSInt)(
        WORD_LEN * 2 - 1,
        WORD_LEN
      ),
      (exe_fun === ALU_MULHSU) -> (op1_data.asSInt * op2_data.asUInt)(
        WORD_LEN * 2 - 1,
        WORD_LEN
      ),
      (exe_fun === ALU_MULHU) -> (op1_data.asUInt * op2_data.asUInt)(
        WORD_LEN * 2 - 1,
        WORD_LEN
      ),
      (exe_fun === ALU_DIVU) -> (op1_data.asUInt / op2_data.asUInt),
      (exe_fun === ALU_REM) -> (op1_data % op2_data),
      (exe_fun === ALU_REMU) -> (op1_data.asUInt % op2_data.asUInt)
    )
  )
